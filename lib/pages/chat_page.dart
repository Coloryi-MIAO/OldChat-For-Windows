...
class _ChatPageState extends State<ChatPage>
  ...
  void _startPolling() {
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
      ...
      _pollGeneration = generation;
      try {
        final api = ApiService();
        final result = widget.type == 'group'
            ? await api.getGroupMessagesAfter(
                widget.conversationId, _lastGroupSeq)
        ...
        if (!mounted || !_isVisible || _pollGeneration != generation) return;
        final newMsgs =
            (result['messages'] as List<Message>?) ?? const <Message>[];
        final newOnes = <Message>[];
        for (final message in newMsgs) {
        ...
        if (newOnes.isNotEmpty) {
          setState(() {
            for (final message in newOnes) {
              final insertAt = _messages.indexWhere((existing) =>
                  _compareMessages(existing, message) > 0);
              if (insertAt < 0) {
                _messages.add(message);
              } else {
          ...
          if (_isUserAtBottom) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted && _isVisible) _scheduleScrollToBottom();
            });
          }
        }
      } catch (_) {}
